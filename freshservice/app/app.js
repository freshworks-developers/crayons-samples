...
function onLoadClickEventHandler() {
  document.getElementById('createTicket').addEventListener('click', function () {
    ...
    if (title && desc && email) {
      createFreshserviceTicket(title, desc, email);
    } else {
...
// 1.2 Paste the code for createFreshserviceTicket() here 👇🏼 !
/**
* 1.2 Function to create a Freshservice ticket ! 🤘
...
function createFreshserviceTicket(title, description, email) {
  client.iparams.get("freshservice_subdomain").then(function (iparam) {
    client.request.post(`https://${iparam.freshservice_subdomain}.freshservice.com/api/v2/tickets`, {
      headers: {
      ...
      .catch(function (error) {
        console.error('Error: Failed to create Freshservice ticket');
        console.error(error);
